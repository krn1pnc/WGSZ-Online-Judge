<?php
	...
	class SyncProblemDataHandler {
		...
		private function compile_at_prepare($name, $config = array()) {
			...
			$cmd_prefix = "$uojMainJudgerWorkPath/run/run_program >{$this->prepare_dir}/run_compiler_result.txt --in=/dev/null --out=stderr --err={$this->prepare_dir}/compiler_result.txt --tl=10 --ml=512 --ol=64 --type=compiler --unsafe --work-path={$work_path}";
